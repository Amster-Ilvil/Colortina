...
def tracked_files() -> list[str]:
    ...
    if proc.returncode != 0: raise SystemExit("privacy audit requires a git checkout")
...
def path_violations(paths: list[str]) -> list[str]:
    ...
    for raw in paths:
        ...
        if name.startswith(".env") and name not in ALLOWED_ENV_TEMPLATES: problems.append(f"forbidden tracked environment file: {raw}")
        if name in FORBIDDEN_NAMES: problems.append(f"forbidden tracked credential/local file: {raw}")
        if parts & FORBIDDEN_DIRS: problems.append(f"forbidden tracked runtime/generated path: {raw}")
        if p.suffix.lower() in FORBIDDEN_SUFFIXES: problems.append(f"forbidden tracked private/runtime file: {raw}")
        if p.suffix.lower() in MODEL_SUFFIXES and parts & MODEL_DIR_HINTS: problems.append(f"model weight/cache must not be tracked: {raw}")
    return problems
    ...
    for raw in paths:
        ...
        for label,pattern in (("macOS home path",MAC_HOME),("Linux home path",LINUX_HOME),("Windows home path",WIN_HOME)):
            for match in pattern.finditer(text):
                a=text.rfind("\n",0,match.start())+1; b=text.find("\n",match.end()); b=len(text) if b<0 else b
                line=text[a:b].lower()
                if "re.compile" in line or "regex" in line or "pattern" in line: continue
                problems.append(f"possible private {label} in tracked file: {raw}"); break
    return problems
