...
def concatenate_input_noise_map(input, noise_sigma):
    ...
    # Fill the downsampled image with zeros — must live on the SAME device
    # as `input` (the old cuda/cpu-only type-string check below left this
    # stuck on CPU under MPS, causing a "Passed CPU tensor to MPS op" crash).
    downsampledfeatures = torch.zeros(
        N, Cout, Hout, Wout, dtype=input.dtype, device=input.device)
...
class UpSampleFeaturesFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # Build output
        grad_input = torch.zeros((N, Cg_in, Hg_in, Wg_in), dtype=grad_output.dtype,
