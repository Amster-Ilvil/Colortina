...
## MangaLineExtraction 漫画线条模型

矩形工具勾选“仅填充选区内闭合区域”时，会使用官方 `MangaLineExtraction_PyTorch` 的 `erika.pth`。首次使用自动从 GitHub Release 下载约 170 MB 到：

```text
models/weights/erika.pth
```

该模型只识别原始黑白图的局部结构线，不参与 mc-v2 自动上色。下载失败时可手动把官方 `erika.pth` 放到上述路径。
